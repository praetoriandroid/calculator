...
public:
    ...
    expression *parse() {
        try {
            tokens = tokenize();
            if (tokens.empty()) {
                handle_error(0, "Empty input");
            }
            return parse_range(0, tokens.size() - 1, priority::lowest);
        } catch (int index) {
            handle_error(index);
        } catch (const parse_exception &e) {
            handle_error(e.start_position, e.what());
        }
    }
...
#pragma clang diagnostic pop
    ...
    void handle_error(int formula_index, const std::string &error_message = "") __attribute__((noreturn)) {
        std::cerr << "Invalid input:" << std::endl;
        std::cerr << '"' << formula << '"' << std::endl;
        std::cerr << std::string(formula_index + 1, ' ') << '^' << std::endl;
...
int main() {
    ...
    std::cout << expression->calc() << std::endl;
    delete expression;
